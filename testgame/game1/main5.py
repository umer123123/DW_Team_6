...
# Load player images
player_normal = "player"  # Normal player image
player_colorblind = "cb_player"  # Colorblind player image
...
def on_key_down(key):
    ...
    if key == keys.C:  # Toggle colorblind mode
        ...
        # Update existing obstacles and player to the new type
        for obstacle in obstacles:
            if colorblind_mode:
                if obstacle_type == 0:
        ...
        # Update player image
        player.image = player_colorblind if colorblind_mode else player_normal


pgzrun.go()

# character change with obstacles