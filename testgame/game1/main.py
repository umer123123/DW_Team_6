...
def update():
    ...
    if spawn_counter >= obstacle_spawn_time:
        spawn_counter = 0
        if obstacle_type == 0:
            obstacles.append(Actor("obstacle_small", (WIDTH, HEIGHT - 60)))  # First obstacle
            obstacle_type = 1  # Next time, spawn second obstacle
        else:
            obstacles.append(Actor("obstacle2_small", (WIDTH, HEIGHT - 60)))  # Second obstacle
            obstacle_type = 0  # Next time, spawn first obstacle
...
def draw():
    ...
    screen.surface.blit(background, (0, 0))  # Draw background
...
def on_key_down(key):
    ...
    global game_started, game_over, is_running_sound_playing
